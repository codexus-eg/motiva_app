...
class LocationDcTab extends StatefulWidget {
  final VoidCallback onContinue;

  const LocationDcTab({super.key, required this.onContinue});

  @override
  State<LocationDcTab> createState() => _LocationDcTabState();
}

class _LocationDcTabState extends State<LocationDcTab> {
  ...
  Future<void> pickLocation() async {
    ...
    try {
      GeoPoint? pickedPoint = await showSimplePickerLocation(
        ...
        context: context,
        title: Translations.of(context).sell_your_car.location_tab.select_location_title,
        textConfirmPicker: Translations.of(context).sell_your_car.location_tab.select,
        textCancelPicker: Translations.of(context).sell_your_car.location_tab.cancel,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${Translations.of(context).sell_your_car.location_tab.failed_picker} ${e.message}')),
        );
      }
    } on Exception catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${Translations.of(context).sell_your_car.location_tab.failed_picker} $e')),
        );
      }
    }
  }
      ...
      child: Column(
        children: [
          CustomDropdown<String>(
            hintText: Translations.of(context).sell_your_car.location_tab.country,
          ...
          Spacer(),
          GradientButton(text: Translations.of(context).sell_your_car.location_tab.kContinue, onTap: widget.onContinue),
          Gap(AppSpacing.xl),
        ],
      ),
    );
